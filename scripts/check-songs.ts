...
async function checkSong(song: Song) {
	...
	const titleMatch =
		found && result.trackName?.toLowerCase() === song.t.toLowerCase();
	const artistMatch =
		found && result.artistName?.toLowerCase() === song.a.toLowerCase();
	...
	for (let i = 0; i < songs.length; i++) {
		...
		if (!result.found) {
			...
			if (!result.artistMatch) {
				console.log(
					`   Artist: "${result.song.a}" vs "${result.fetchedArtist}"`,
				);
			}
			if (!result.yearMatch)
				console.log(`   Year: ${result.song.y} vs ${result.fetchedYear}`);
		} else if (!result.hasPreview || !result.hasArtwork) {
			console.log(
				`[${i + 1}/${songs.length}] ⚠️  ${result.song.t} - ${result.song.a}`,
			);
			console.log(
				`   Preview: ${result.hasPreview}, Artwork: ${result.hasArtwork}`,
			);
		} else {
			console.log(
				`[${i + 1}/${songs.length}] ✓ ${result.song.t} - ${result.song.a}`,
			);
		}
